...
public class Pedido implements Auditavel, Calculavel {
    ...
    public String obterHistorico() {
        ...
        sb.append("========== HISTÓRICO DE AUDITORIA DO PEDIDO #").append(id).append(" ==========\n");
    ...
    /**
     * Calcula a taxa de entrega com base no valor do pedido.
     * Regra de negócio: Desconto progressivo na taxa de entrega conforme valor do pedido.
     */
    private double calcularTaxaEntrega(double subtotal, double desconto) {
        double valorFinal = subtotal - desconto;
        
        if (valorFinal >= 150) {
            return 5.0;  // Taxa reduzida para pedidos acima de R$150
        } else if (valorFinal >= 100) {
            return 7.0;  // Taxa moderada para pedidos entre R$100 e R$150
        } else {
            return 10.0; // Taxa padrão para pedidos menores
        }
    }
}
